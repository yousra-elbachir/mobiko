...
Strategy (pure text-matching, punctuation excluded):
...
Every result carries a `status` so unresolved matches can be surfaced for manual
adjustment.
"""
...
# quotes/dashes/ellipsis. Internal punctuation (e.g. "Earth's surface") is kept;
# only outer edges are trimmed.
...
def _token_overlap(sentence: str, query: str) -> Optional[Tuple[int, int]]:
    ...
    Handles cases where the model lightly rephrased the entity (extra article,
    plural) but a contiguous core still appears in the text. Prefers the longest
    match at the widest token window, and never returns a lone stopword.
    """
...
def resolve_span(entity_text: str, sentence: str) -> dict:
    ...
    Returns {text, start_char, end_char, status}. When unresolved, start_char/
    end_char are -1 and `text` is the cleaned query for display.
    """
    ...
    # 4. Give up; the span can be set manually.
    return {"text": query, "start_char": -1, "end_char": -1,
            "status": STATUS_UNRESOLVED}
